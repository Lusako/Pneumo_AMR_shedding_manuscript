...
pvalue <- 
  ...
  ungroup() %>%
  mutate(new = if_else(hivst == "NVT, HIV-", "pos",
                       if_else(hivst == "NVT, PLHIV>1yr", "pos", "neg", NA_character_)))

table(pvalue$HIV, pvalue$new)
chisq.test(pvalue$new, pvalue$HIV)
a <- tibble::tribble(
  ~group1, ~group2,   ~p.adj,
  "NVT, HIV-",     "NVT, PLHIV>1yr", 9.664e-05,
  "VT, HIV-",     "VT, PLHIV>1yr", 0.9798)
...
a <- tibble::tribble(
  ~group1, ~group2,   ~p.adj,
  "HIV-",     "PLHIV>1yr", 0.0103)

E<- 
  pneumoshed %>% 
  group_by(chiv_status, aero_shed) %>%
  filter(!is.na(aero_shed), Carriage == "carriage") %>%
  tally() %>%
  mutate(N = sum(n), prev = n/sum(n)) %>%
  rowwise() %>%
  mutate(obs_lci = exactci(n, N, 0.95)$conf.int[1], 
